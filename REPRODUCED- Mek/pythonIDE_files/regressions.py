...
class RDataFrame(pd.DataFrame):
    ...
    def xtreg(self, formula, regression_type='fe', vce='nonrobust', cluster=None, verbose=True):
        self.check_panel()
        result = Regression(formula=formula, data=self, regression_type=regression_type, vce=vce, cluster=cluster)
        if verbose:
            print(result.summary)
        return result
...
class Regression:
    ...
    def _fit(self,hyp):
        model = smf.ols(self.formula, self.data)
        ...
        #fTest = model.fit(cov_type=self.vce, cov_kwds=cov_kwds, use_t = True).f_test(hyp)
        #pValue = fTest.pvalue.item()
        #print(pValue)
        return model.fit(cov_type=self.vce, cov_kwds=cov_kwds, use_t = True) 
